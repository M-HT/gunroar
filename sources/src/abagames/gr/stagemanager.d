  ...
  public void addBatteries(PlatformPos[] platformPos, int platformPosNum) {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      if (!platformEnemySpec.setFirstState(en.state, p.x, p.y, platformPos[ppi].deg))
        continue;
      for (int i1 = 0; i1 < platformPosNum; i1++) {
        if (fabs(platformPos[ppi].pos.x - platformPos[i1].pos.x) <= 1 &&
            fabs(platformPos[ppi].pos.y - platformPos[i1].pos.y) <= 1 &&
            !platformPos[i1].used) {
          platformPos[i1].used = true;
          ppn--;
        }
      }
